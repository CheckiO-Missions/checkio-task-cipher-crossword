...
requirejs(['ext_editor_1', 'jquery_190', 'raphael_210'],
    function (ext, $, TableComponent) {
        ...
        ext.set_animate_slide(function (this_e, data, options) {
            ...
            if (!result) {
                $content.find('.call').html('Fail: checkio(' + ext.JSON.stringify(checkioInput[0]) + "," +
                    ext.JSON.stringify(checkioInput[0])+ ')');
                ...
                $content.find('.call').html('Pass: checkio(' + ext.JSON.stringify(checkioInput[0]) + "," +
                    ext.JSON.stringify(checkioInput[0])+ ')');
                $content.find('.answer').remove();
            }
